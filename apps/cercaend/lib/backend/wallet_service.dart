...
class WalletCreationResult {
  ...
  final String seedPhraseHash;
  final String seedPhrase; // Only for immediate display, not stored

  WalletCreationResult({
    required this.walletAddress,
    required this.encryptedPrivateKey,
    required this.seedPhraseHash,
    required this.seedPhrase,
  });
}

/// Wallet generation and management service
class WalletService {
  static const String _bip39WordList = '''
...
any apart apology appear apple apply arctic area argue arise army
around arrange arrest arrive arrow art artist artwork ask aspect
...
charity charm chart chase cheap check cheese chef cherry chest
...
defense define defy degree delay deliver demand demise denial dense
dent depend deposit depth deputy derive describe desert design desk
...
drama drastic draw dream dress drill drink drip drive drop drum dry
duck dumb dune during dust duty each eagle ear early earn earth
easily east easy echo ecology economy edge edit educate effort egg
...
enhance enjoy enlist enough enrich enroll entire entry envelope
...
extend extra eye eyebrow fabric face faculty fade faint faith fall
false fame family famous fan fancy fantasy farm fashion fat fatal
...
hour hover hub huge human humble humor hundred hungry hunt hurdle
hurry hurt husband hybrid ice icon idea identify idle ignore ill
illegal illness image imitate immense immune impact impose improve impulse
inch include income increase index indoor industry infant inflict inform
...
luggage lumber lunar lunch luxury lyrics machine magic magnet maid mail
main major make mammal man manage mandate mango mansion manual maple
...
merge merit merry mesh message metal method middle midnight milk million
mind mine mirror misery miss mistake mix mixed mixture mobile model
...
parse part participant party pass patch path patient patrol pattern
pause pave payment peace peanut pear peasant pelican pen penalty pencil
people pepper perfect permit person phone photo phrase physical piano
...
portion position possible post potato powder power practice praise
predict prefer prepare present pretty prevent price pride primary print
priority prize private prize problem process produce profit program
...
purse push put puzzle python quality quantum quarter question quick
...
response result retreat return reunion reveal review reward rhythm
...
run runway rural sad safe sail salad salmon salon salt salute same
sample sand satisfy satoshi sauce sausage save say scale scan scare
...
shell sheriff shield shift shine shock shoot short shoulder shove shrimp
...
slight slim slogan slot slow slush small smart smile snake snap
...
south space spare spatial spawn speak special speed spend sphere spice
spider spike spin spirit split spoken sport spot spray spread spring
spy square squeeze squirrel stable staff stage stairs stamp stand
start state stay steak steel steep stem step stereo stick still
sting stock stone stool story stove strategy street strike strong
...
sustain swallow swamp swap swarm swear sweet swift swim swing switch
sword symbol symptom syrup system table tackle tag tail talent talk
tank tape target task taste tattoo taxi teach team tell ten tennis
...
trip triumph trivia trophy trouble truck true truly trumpet trust
...
version very vessel veteran viable vibrant vicious victory video
view village vintage violin virtual virus visa visit visual vital
vocal voice void volcano volume vote voyage wage waist wait wake
walk wall walnut want warfare warm warrior wash wasp waste water
...
wide width wife wild will win window wine wing winter wire wisdom
...
youth zero zone zoo''';
  ...
  static Future<WalletCreationResult> createWalletForUser({
    ...
    try {
      ...
      final keyPair = await _generateECDSAKeyPair();

      // Generate 12-word seed phrase (BIP39 style)
      final seedPhrase = _generateSeedPhrase();

      // Create wallet address from public key
      final walletAddress = await _deriveAddressFromPublicKey(keyPair.publicKey);

      // Hash the seed phrase for storage (not the actual phrase)
      final seedHash = _hashSeedPhrase(seedPhrase);
